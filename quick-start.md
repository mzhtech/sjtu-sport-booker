...
并发模式只执行一次 JAccount 登录，随后把已认证会话复制给其他浏览器。目标日期会随机且无重复地分配给各并发；分配数量差最多 1。并发度大于日期数时，只启动与日期数相同的浏览器。

任务由全流程监督器持续运行。浏览器驱动、登录、会话恢复、场馆扫描或页面刷新发生异常时，当前浏览器组会全部关闭，短暂等待后重新登录并重新开始；只有抢票成功或手动点击停止才会结束。
